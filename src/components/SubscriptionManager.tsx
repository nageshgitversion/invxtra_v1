...
export default function SubscriptionManager({ transactions }: SubscriptionManagerProps) {
  ...
  const subscriptions = useMemo(() => {
    // A subscription is a recurring expense OR a transaction with common subscription keywords
    const subKeywords = ['netflix', 'spotify', 'youtube', 'amazon', 'prime', 'disney', 'hotstar', 'gym', 'internet', 'broadband', 'insurance', 'icloud', 'apple', 'google', 'subscription', 'membership'];
    
    const explicitSubs = transactions.filter(t => t.isRecurring && t.type === 'expense');
    
    // Potentially forgot to mark as recurring
    const potentialSubs = transactions.filter(t => 
      !t.isRecurring && 
      t.type === 'expense' && 
      subKeywords.some(kw => t.name.toLowerCase().includes(kw))
    );

    // Combine and remove duplicates by ID or name
    const combined = [...explicitSubs, ...potentialSubs];
  ...
  if (subscriptions.length === 0) {
    return (
      <div className="glass-card p-6 rounded-[32px] border-dashed border-2 border-indigo-100 bg-indigo-50/20">
        <div className="flex items-center gap-3 mb-4">
          <div className="w-10 h-10 rounded-xl bg-white text-indigo-400 flex items-center justify-center">
            <Mail size={20} />
          </div>
          <div>
            <h3 className="font-display font-bold text-lg text-slate-400">Subscription Negotiator</h3>
            <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">No recurring expenses found</p>
          </div>
        </div>
        <p className="text-xs text-slate-500 font-medium leading-relaxed">
          Mark your Netflix, Spotify, or Gym memberships as <span className="text-indigo-600 font-bold">"Recurring"</span> in the Transactions tab to enable AI-powered negotiation and bill tracking.
        </p>
      </div>
    );
  }
